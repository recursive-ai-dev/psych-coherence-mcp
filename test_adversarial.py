...
async def test_injection_attacks():
    ...
    for i, payload in enumerate(injection_payloads):
        try:
            ...
            check(f"Injection {i+1} not executed", 
...
async def test_extreme_inputs():
    ...
    try:
        result = json.loads(await psy_analyze_input(AnalyzeInputModel(text="")))
    ...
    start = time.time()
    try:
        result = json.loads(await psy_analyze_input(AnalyzeInputModel(text=long_text)))
    ...
    for i, text in enumerate(pattern_tests):
        ...
        try:
            result = json.loads(await psy_analyze_input(AnalyzeInputModel(text=text)))
            elapsed = time.time() - start
            check(f"Pattern test {i+1} completes", elapsed < 5, f"Took {elapsed:.2f}s", critical=True)
        except Exception as e:
...
async def test_state_corruption():
    ...
    try:
        result = json.loads(await psy_create_session(CreateSessionInput(
    ...
    try:
        result = json.loads(await psy_get_coherence_state(SessionIdInput(session_id="nonexistent-xyz")))
        check("Non-existent session returns error/graceful", 
    ...
    try:
        ...
        check("Double end-session handled", 
    ...
    try:
        ...
        check("Store to ended session rejected", 
...
async def test_memory_poisoning():
    ...
    await psy_create_session(CreateSessionInput(persona_id="counselor_amara", session_id=session_id))
    ...
    for content, mtype, imp in conflicting_memories:
        try:
            ...
            check(f"Conflicting memory stored", False, str(e))
    
    # Check recall doesn't crash with conflicts
    try:
        result = json.loads(await psy_recall(RecallInput(
            session_id=session_id,
            query="sky",
            max_results=10
        )))
...
async def test_belief_contradiction_exploitation():
    ...
    for entity, attr, value, conf in contradictions:
        try:
            ...
            check(f"Contradictory belief stored", False, str(e))
    ...
    try:
        ...
        check("Contradictions detected", len(contradictions_detected) > 0, 
    ...
    try:
        tasks = []
        for i in range(50):
            tasks.append(psy_store_belief(StoreBeliefInput(
                session_id=session_id,
                entity="rapid_test",
                attribute="counter",
                value=str(i),
...
async def test_personality_manipulation():
    ...
    for i, text in enumerate(extreme_texts):
        try:
            ...
            check(f"Extreme text {i+1} produces valid profile", 
    ...
    try:
        ...
        check("Mixed signals reduce confidence", 
...
async def test_emotion_detection_edge_cases():
    ...
    for i, text in enumerate(neutral_texts):
        try:
            ...
            check(f"Neutral text {i+1} has low intensity", 
...
async def test_generation_constraints():
    ...
    await psy_create_session(CreateSessionInput(persona_id="storyteller_vex", session_id=session_id))
    ...
    try:
        ...
        has_expected_keys = any(k in result for k in ["persona_name", "tone_directives", "constraints"])
        check("Long input constraints built", has_expected_keys, f"Got keys: {list(result.keys())[:5]}")
    except Exception as e:
...
async def test_type_confusion():
    ...
    for i, content in enumerate(type_confusions):
        try:
            ...
            check(f"Type confusion {i+1} ({type(content).__name__}) stored", "memory_id" in result)
        except Exception as e:
...
async def main():
    ...
    if CRITICAL_FAIL > 0:
        ...
        print(f"\n  ⚠️  {FAIL} failures found, but no critical vulnerabilities")
        return 0
    else:
