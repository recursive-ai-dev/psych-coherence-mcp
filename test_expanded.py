...
async def main() -> None:
    ...
    moderate = json.loads(await psy_assess_safety(SafetyAssessmentInput(text="I want to kill myself.")))
    assert moderate["risk_level"] == "moderate"
    imminent = json.loads(await psy_assess_safety(SafetyAssessmentInput(
    ...
    protected = json.loads(await psy_assess_safety(SafetyAssessmentInput(
    ...
    extraction = json.loads(await psy_extract_memories(AnalyzeInputModel(
    ...
    created = json.loads(await psy_create_session(CreateSessionInput(
        persona_id="engineer_kai", session_id="expanded-test"
    )))
    assert created["status"] == "active"

    brief = json.loads(await psy_generate_response(GenerateResponseInput(
    ...
    recorded = json.loads(await psy_record_response(RecordResponseInput(
        session_id="expanded-test",
        response_text=("Are you in immediate danger, and are you safe right now? "
    ...
    imported = json.loads(await psy_import_session(ImportSessionInput(
        snapshot=snapshot, new_session_id="expanded-restored"
    )))
    assert imported["status"] == "imported"
    assert imported["turn_count"] == 1

    sessions = json.loads(await psy_list_sessions())
    assert {item["session_id"] for item in sessions["sessions"]} >= {"expanded-test", "expanded-restored"}

    punctuation = json.loads(await psy_humanize_text(HumanizeInput(text=".", persona_id="engineer_kai")))
